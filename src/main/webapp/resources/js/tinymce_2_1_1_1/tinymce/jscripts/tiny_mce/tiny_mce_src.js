 ...
 * Licence Agreement:
 * 
 * This program is a Commercial licensed software. You are not authorized to
 * redistribute it and/or modify/and or sell it under any publication either
 * user and enterprise versions of the License (or) any later version is
 * applicable for the same. If you have received this software without a
 * license, you must not use it, and you must destroy your copy of it
 * immediately. If anybody illegally uses this software, please contact
 * info@ajsquare.com.
 * 
 ******************************************************************************/
...
TinyMCE_Engine.prototype = {
	init : function(settings) {
		...
		if (!tinyMCE.baseURL) {
			...
			for (i = 0; i < elements.length; i++) {
				if (elements[i].src
						&& (elements[i].src.indexOf("tiny_mce.js") != -1
								|| elements[i].src.indexOf("tiny_mce_dev.js") != -1
								|| elements[i].src.indexOf("tiny_mce_src.js") != -1 || elements[i].src
								.indexOf("tiny_mce_gzip") != -1)) {
					src = elements[i].src;
					tinyMCE.srcMode = (src.indexOf('_src') != -1 || src
							.indexOf('_dev') != -1) ? '_src' : '';
					tinyMCE.gzipMode = src.indexOf('_gzip') != -1;
					src = src.substring(0, src.lastIndexOf('/'));
					if (settings.exec_mode == "src"
							|| settings.exec_mode == "normal")
		...
		if (tinyMCE.baseURL.indexOf('://') == -1
				&& tinyMCE.baseURL.charAt(0) != '/') {
		...
		this._def("encoding", "");
		this
				._def("urlconverter_callback", tinyMCE.getParam(
		...
		this.settings.base_href = baseHREF.substring(0, baseHREF
				.lastIndexOf('/'))
				+ "/";
		...
		this.callbacks = [ 'onInit', 'getInfo', 'getEditorTemplate',
	...
	hasPlugin : function(n) {
		return typeof (this.plugins[n]) != "undefined"
				&& this.plugins[n] != null
	},
	...
	confirmAdd : function(e, settings) {
		...
		if (tinyMCE.settings.convert_on_click
				|| (!elm.getAttribute('mce_noask') && confirm(tinyMCELang.lang_edit_confirm)))
	...
	updateContent : function(form_element_name) {
		...
		for (n in tinyMCE.instances) {
			...
			if (inst.formElement == formElement) {
				...
				if (!tinyMCE.isIE)
					doc.body.innerHTML = tinyMCE._cleanupHTML(inst, doc,
	...
	execInstanceCommand : function(editor_id, command, user_interface, value,
			focus) {
	...
	setupContent : function(editor_id) {
		var inst = tinyMCE.instances[editor_id], i, doc = inst.getDoc(), head = doc
		...
		if (!tinyMCE.isIE) {
			...
			if (tinyMCE.settings.cleanup_on_startup)
				tinyMCE.setInnerHTML(inst.getBody(), tinyMCE._cleanupHTML(inst,
						doc, this.settings, contentElement));
			else
			...
			if (tinyMCE.settings.cleanup_on_startup) {
				...
				try {
					tinyMCE.setInnerHTML(inst.getBody(), tinyMCE._cleanupHTML(
							inst, inst.contentDocument, this.settings, inst
									.getBody()))
				} catch (e) {
				}
			} else
		...
		if (tinyMCE.isGecko) {
			tinyMCE.selectNodes(inst.getBody(), function(n) {
				if (n.nodeType == 3 || n.nodeType == 8)
					n.nodeValue = n.nodeValue.replace(new RegExp(
							'\\s(mce_src|mce_href)=\"[^\"]*\"', 'gi'), "");
				return false
			})
		}
		if (tinyMCE.isGecko)
	...
	storeAwayURLs : function(s) {
		if (!s.match(/(mce_src|mce_href)/gi, s)) {
			s = s.replace(new RegExp('src\\s*=\\s*\"([^ >\"]*)\"', 'gi'),
					'src="$1" mce_src="$1"');
			s = s.replace(new RegExp('href\\s*=\\s*\"([^ >\"]*)\"', 'gi'),
					'href="$1" mce_href="$1"')
		}
		return s
	},
	...
	removeTinyMCEFormElements : function(form_obj) {
		...
		for (i = 0; i < form_obj.elements.length; i++) {
			elementId = form_obj.elements[i].name ? form_obj.elements[i].name
	...
	handleEvent : function(e) {
		...
		case "beforepaste":
			...
			window
					.setTimeout(
							"tinyMCE.selectedInstance.setBaseHREF(tinyMCE.settings.base_href);tinyMCE._resetIframeHeight();",
							1);
			return;
		case "submit":
		...
		case "keypress":
			...
			if ((tinyMCE.isGecko || tinyMCE.isOpera || tinyMCE.isSafari)
					...
					&& !e.shiftKey) {
				if (TinyMCE_ForceParagraphs._insertPara(
						tinyMCE.selectedInstance, e)) {
					tinyMCE.execCommand("mceAddUndoLevel");
					return tinyMCE.cancelEvent(e)
				}
			}
			if ((tinyMCE.isGecko && !tinyMCE.isSafari)
					&& tinyMCE.settings.force_p_newlines
					&& (e.keyCode == 8 || e.keyCode == 46) && !e.shiftKey) {
				if (TinyMCE_ForceParagraphs._handleBackSpace(
						tinyMCE.selectedInstance, e.type)) {
					tinyMCE.execCommand("mceAddUndoLevel");
					return tinyMCE.cancelEvent(e)
				}
			}
		...
		case "keydown":
			...
			if (tinyMCE.isGecko && e.type == "keyup" && e.keyCode == 9)
				tinyMCE
						.handleVisualAid(tinyMCE.selectedInstance.getBody(),
								true, tinyMCE.settings.visual,
								tinyMCE.selectedInstance);
			if (tinyMCE.isIE && e.type == "keydown" && e.keyCode == 13)
				tinyMCE.enterKeyElement = tinyMCE.selectedInstance
						.getFocusElement();
		...
		case "focus":
			...
			for ( var instanceName in tinyMCE.instances) {
				...
				if (inst.getBody().parentNode == targetBody) {
					inst.select();
					tinyMCE.selectedElement = e.target;
					tinyMCE.linkElement = tinyMCE.getParentElement(
							tinyMCE.selectedElement, "a");
					tinyMCE.imgElement = tinyMCE.getParentElement(
							tinyMCE.selectedElement, "img");
					break
				}
			}
			if (!tinyMCE.selectedInstance.undoRedo.undoLevels[0].bookmark
					&& (e.type == "mouseup" || e.type == "dblclick"))
				tinyMCE.selectedInstance.undoRedo.undoLevels[0].bookmark = tinyMCE.selectedInstance.selection
						.getBookmark();
			if (e.type != "focus")
				tinyMCE.selectedNode = null;
			tinyMCE.triggerNodeChange(false);
	...
	getButtonHTML : function(id, lang, img, cmd, ui, val) {
		...
		if (tinyMCE.getParam('button_tile_map')
				&& (!tinyMCE.isIE || tinyMCE.isOpera)
				&& (m = this.buttonMap[id]) != null
				&& (tinyMCE.getParam("language") == "en" || img
						.indexOf('$lang') == -1)) {
			x = 0 - (m * 20) == 0 ? '0' : 0 - (m * 20);
			h += '<a id="{$editor_id}_' + id + '" href="javascript:' + cmd
					+ '" onclick="' + cmd
					+ 'return false;" onmousedown="return false;" ' + io
					+ ' class="mceTiledButton mceButtonNormal" target="_self">';
			h += '<img src="{$themeurl}/images/spacer.gif" style="background-position: '
					+ x
					+ 'px 0" alt="{$'
					+ lang
					+ '}" title="{$'
	...
	getMenuButtonHTML : function(id, lang, img, mcmd, cmd, ui, val) {
		...
		if (tinyMCE.getParam('button_tile_map')
				&& (!tinyMCE.isIE || tinyMCE.isOpera)
				&& (m = tinyMCE.buttonMap[id]) != null
				&& (tinyMCE.getParam("language") == "en" || img
						.indexOf('$lang') == -1)) {
			x = 0 - (m * 20) == 0 ? '0' : 0 - (m * 20);
			if (tinyMCE.isRealIE)
				h += '<span id="{$editor_id}_'
						+ id
						+ '" class="mceMenuButton" onmouseover="tinyMCE._menuButtonEvent(\'over\',this);tinyMCE.lastHover = this;" onmouseout="tinyMCE._menuButtonEvent(\'out\',this);">';
			else
			...
			h += '<a href="javascript:'
					...
					+ '" onclick="'
					+ cmd
					+ 'return false;" onmousedown="return false;" class="mceTiledButton mceMenuButtonNormal" target="_self">';
			h += '<img src="{$themeurl}/images/spacer.gif" style="width: 20px; height: 20px; background-position: '
					+ x + 'px 0" title="{$' + lang + '}" /></a>';
			h += '<a href="javascript:'
					+ mcmd
					+ '" onclick="'
					+ mcmd
					+ 'return false;" onmousedown="return false;"><img src="{$themeurl}/images/button_menu.gif" title="{$'
					+ lang + '}" class="mceMenuButton" />';
			h += '</a></span>'
		} else {
			if (tinyMCE.isRealIE)
				h += '<span id="{$editor_id}_'
						+ id
						+ '" dir="ltr" class="mceMenuButton" onmouseover="tinyMCE._menuButtonEvent(\'over\',this);tinyMCE.lastHover = this;" onmouseout="tinyMCE._menuButtonEvent(\'out\',this);">';
			else
				h += '<span id="{$editor_id}_' + id
						+ '" dir="ltr" class="mceMenuButton">';
			h += '<a href="javascript:'
					+ cmd
					+ '" onclick="'
					+ cmd
					+ 'return false;" onmousedown="return false;" class="mceMenuButtonNormal" target="_self">';
	...
	onLoad : function() {
		...
		for (c = 0; c < tinyMCE.configs.length; c++) {
			...
			case "exact":
				...
				for (i = 0; i < elements.length; i++) {
					...
					trigger = element ? element
							.getAttribute(tinyMCE.settings.textarea_trigger)
							: "";
					if (new RegExp('\\b' + deselector + '\\b').test(tinyMCE
							.getAttrib(element, "class")))
						continue;
					if (trigger == "false")
						continue;
				...
				for (i = 0; i < elements.length; i++) {
					...
					if (selector !== ''
							&& !new RegExp('\\b' + selector + '\\b')
									.test(tinyMCE.getAttrib(elm, "class")))
						continue;
					if (selector !== '')
					...
					if (tinyMCE.isGecko) {
						...
						if (element.nodeName != "TEXTAREA"
							...
							tinyMCE.addEvent(element, "click", function(e) {
								window.setTimeout(function() {
									TinyMCE_Engine.prototype.confirmAdd(e,
											settings)
								}, 10)
							});
					} else {
			...
			if (tinyMCE.settings.auto_focus) {
				window.setTimeout(function() {
					var inst = tinyMCE
							.getInstanceById(tinyMCE.settings.auto_focus);
	...
	getParam : function(name, default_value, strip_whitespace, split_chr) {
		var i, outArray, value = (typeof (this.settings[name]) == "undefined") ? default_value
				: this.settings[name];
	...
	addToLang : function(prefix, ar) {
		...
		for (k in ar) {
			if (typeof (ar[k]) == 'function')
				continue;
			tinyMCELang[(k.indexOf('lang_') == -1 ? 'lang_' : '')
					+ (prefix !== '' ? (prefix + "_") : '') + k] = ar[k]
		}
		this.loadNextScript()
	},
	triggerNodeChange : function(focus, setup_content) {
		...
		if (tinyMCE.selectedInstance) {
			inst = tinyMCE.selectedInstance;
			elm = (typeof (setup_content) != "undefined" && setup_content) ? tinyMCE.selectedElement
					: inst.getFocusElement();
	...
	_customCleanup : function(inst, type, content) {
		...
		if (customCleanup != '')
			content = tinyMCE.resolveDots(tinyMCE.settings.cleanup_callback,
	...
	openWindow : function(template, args) {
		...
		if (html) {
			...
			win = window.open("", "mcePopup" + new Date().getTime(), "top=" + y
					+ ",left=" + x + ",scrollbars=" + scrollbars
					+ ",dialog=yes,minimizable=" + resizable
			...
			if ((tinyMCE.isRealIE) && resizable != 'yes'
				...
				height += 10;
				features = "resizable:" + resizable + ";scroll:" + scrollbars
				...
				win = window.open(url, "mcePopup" + new Date().getTime(),
						"top=" + y + ",left=" + x + ",scrollbars=" + scrollbars
								+ ",dialog=" + modal + ",minimizable="
								+ resizable + ",modal=" + modal + ",width="
	...
	handleVisualAid : function(el, deep, state, inst, skip_dispatch) {
		...
		case "TABLE":
			...
			tinyMCE.setAttrib(el, "class", tinyMCE.getVisualAidClass(tinyMCE
					.getAttrib(el, "class"), state && bo));
			el.style.width = oldW;
			el.style.height = oldH;
			for (y = 0; y < el.rows.length; y++) {
				for (x = 0; x < el.rows[y].cells.length; x++) {
					cn = tinyMCE.getVisualAidClass(tinyMCE.getAttrib(
							el.rows[y].cells[x], "class"), state && bo);
		...
		if (tinyMCE.isGecko) {
			if (m == 1) {
				...
				tinyMCE
						.selectElements(
								e,
								'A,IMG,SELECT,AREA,IFRAME,BASE,INPUT,SCRIPT,EMBED,OBJECT,LINK',
								function(n) {
				...
				tinyMCE.selectNodes(e, function(n) {
					if (n.nodeType == 3 || n.nodeType == 8) {
						n.nodeValue = n.nodeValue.replace(/\smce_tsrc=/gi,
								" src=");
						n.nodeValue = n.nodeValue.replace(/\smce_thref=/gi,
								" href=")
					}
					return false
				})
	...
	getWindowArg : function(n, d) {
		return (typeof (this.windowArgs[n]) == "undefined") ? d
				: this.windowArgs[n]
	},
	getCSSClasses : function(editor_id, doc) {
		var i, c, x, rule, styles, rules, csses, selectorText, inst = tinyMCE
		...
		if (styles && styles.length > 0) {
			for (x = 0; x < styles.length; x++) {
				csses = null;
				try {
					csses = tinyMCE.isIE ? doc.styleSheets(x).rules
							: styles[x].cssRules
				} catch (e) {
				}
				if (!csses)
					...
					if (selectorText) {
						...
						for (c = 0; c < rules.length; c++) {
							...
							if (rule.indexOf(' ') != -1
								...
								continue;
							if (rule
									.indexOf(tinyMCE.settings.visual_table_class) != -1
									|| rule.indexOf('mceEditable') != -1
									|| rule.indexOf('mceNonEditable') != -1)
								continue;
							if (rule.indexOf('.') != -1) {
								cssClass = rule
										.substring(rule.indexOf('.') + 1);
								addClass = true;
								for (p = 0; p < inst.cssClasses.length
										&& addClass; p++) {
	...
	getControlHTML : function(c) {
		...
		for (n in l) {
			...
			if (o.getControlHTML && (v = o.getControlHTML(c)) !== '') {
				if (rtl)
					return '<span dir="rtl">'
							+ tinyMCE.replaceVar(v, "pluginurl", o.baseURL)
							+ '</span>';
...
TinyMCE_Control.prototype = {
	...
	resizeToContent : function() {
		...
		this.iframeElement.style.height = (tinyMCE.isRealIE) ? b.scrollHeight
				: de.offsetHeight + 'px'
	},
	addShortcut : function(m, k, d, cmd, ui, va) {
		...
		for (i = 0; i < scl.length; i++) {
			if (sc.alt == scl[i].alt && sc.ctrl == scl[i].ctrl
					&& sc.shift == scl[i].shift
					&& sc.charCode == scl[i].charCode
					&& sc.keyCode == scl[i].keyCode) {
				return false
			}
		}
		scl[scl.length] = sc;
	...
	handleShortcut : function(e) {
		...
		for (i = 0; i < s.length; i++) {
			o = s[i];
			if (o.alt == e.altKey && o.ctrl == e.ctrlKey
					&& (o.keyCode == e.keyCode || o.charCode == e.charCode)) {
				if (o.cmd
						&& (e.type == "keydown" || (e.type == "keypress" && !tinyMCE.isOpera)))
	...
	isDirty : function() {
		return tinyMCE.trim(this.startContent) != tinyMCE
				.trim(this.getBody().innerHTML)
				&& !this.isNotDirty
	},
	...
	execCommand : function(command, user_interface, value) {
		var i, x, z, align, img, div, doc = this.getDoc(), win = this.getWin(), focusElm = this
				.getFocusElement();
		if (!new RegExp(
				'mceStartTyping|mceEndTyping|mceBeginUndoLevel|mceEndUndoLevel|mceAddUndoLevel',
				'gi').test(command))
		...
		if (!/mceStartTyping|mceEndTyping/.test(command)) {
			if (tinyMCE.execCommandCallback(this, 'execcommand_callback',
					'execCommand', this.editorId, this.getBody(), command,
					user_interface, value))
				return
		}
		if (focusElm && focusElm.nodeName == "IMG") {
			align = focusElm.getAttribute('align');
			img = command == "JustifyCenter" ? focusElm.cloneNode(false)
					: focusElm;
			switch (command) {
			case "JustifyLeft":
				if (align == 'left')
				...
				if (div && div.nodeName == "DIV" && div.childNodes.length == 1
						&& div.parentNode)
		...
		if (tinyMCE.settings.force_br_newlines) {
			...
			if (doc.selection.type != "Control") {
				...
				if (alignValue !== '') {
					...
					if ((divElm = tinyMCE.getParentElement(rng.parentElement(),
							"div")) != null)
		...
		case "FormatBlock":
			if (value == null || value == '') {
				var elm = tinyMCE
						.getParentElement(this.getFocusElement(),
				...
				if (!this.cleanup.isValid(value))
					return true;
				if (tinyMCE.isGecko
						&& new RegExp('<(div|blockquote|code|dt|dd|dl|samp)>',
								'gi').test(value))
					value = value.replace(/[^a-z]/gi, '');
				if (tinyMCE.isIE
						&& new RegExp('blockquote|code|samp', 'gi').test(value)) {
					var b = this.selection.getBookmark();
					this.getDoc().execCommand("FormatBlock", false, '<p>');
					tinyMCE.renameElement(tinyMCE.getParentBlockElement(this
		...
		case "SetStyleInfo":
			...
			var invalidParentsRe = tinyMCE.settings.merge_styles_invalid_parents !== '' ? new RegExp(
					tinyMCE.settings.merge_styles_invalid_parents, "gi")
					: null;
			if (tinyMCE.isIE) {
				if (rng.item)
				...
				if (sel.isCollapsed
						|| (new RegExp('td|tr|tbody|table|img', 'gi')
								.test(felm.nodeName) && sel.anchorNode == felm.parentNode))
					parentElm = felm
			}
			if (parentElm && !invalidRe.test(parentElm.nodeName)) {
				...
				var elementArray = tinyMCE.getElementsByAttributeValue(this
						.getBody(), "font", "face", "#mce_temp_font#");
					...
					if (elm) {
						...
						if (elm.hasChildNodes()) {
							for (i = 0; i < elm.childNodes.length; i++)
								spanElm.appendChild(elm.childNodes[i]
										.cloneNode(true))
						}
			...
			for (i = nodes.length - 1; i >= 0; i--) {
				...
				tmp.innerHTML = tmp.innerHTML.replace(new RegExp(
						'style=""|class=""', 'gi'), '');
				if (new RegExp('<span>', 'gi').test(tmp.innerHTML)) {
					for (x = 0; x < elm.childNodes.length; x++) {
						if (elm.parentNode != null)
							elm.parentNode.insertBefore(elm.childNodes[x]
									.cloneNode(true), elm)
					}
		...
		case "FontName":
			if (value == null) {
				...
				if (tinyMCE.isGecko && s.isCollapsed) {
					var f = tinyMCE.getParentElement(this.getFocusElement(),
							"font");
					if (f != null)
		...
		case "Paste":
			...
			if (tinyMCE.isGecko && cmdFailed) {
				if (confirm(tinyMCE.entityDecode(tinyMCE
		...
		case "mceSetContent":
			...
			tinyMCE.setInnerHTML(this.getBody(), tinyMCE._cleanupHTML(this,
					doc, this.settings, this.getBody(), false, false, false,
					true));
		...
		case "mceCleanup":
			...
			tinyMCE.setInnerHTML(this.getBody(), tinyMCE._cleanupHTML(this,
					this.contentDocument, this.settings, this.getBody(),
					this.visualAid));
		...
		case "mceSetAttribute":
			if (typeof (value) == 'object') {
				var targetElms = (typeof (value.targets) == "undefined") ? "p,img,span,div,td,h1,h2,h3,h4,h5,h6,pre,address"
						: value.targets;
				var targetNode = tinyMCE.getParentElement(this
						.getFocusElement(), targetElms);
				if (targetNode) {
		...
		case "mceInsertRawHTML":
			...
			var scrollX = this.getBody().scrollLeft
					+ this.getDoc().documentElement.scrollLeft;
			var scrollY = this.getBody().scrollTop
					+ this.getDoc().documentElement.scrollTop;
			var html = this.getBody().innerHTML;
			if ((pos = html.indexOf(key)) != -1)
				tinyMCE.setInnerHTML(this.getBody(), html.substring(0, pos)
						+ value + html.substring(pos + key.length));
			...
			if (tinyMCE.isGecko || tinyMCE.isOpera) {
				try {
					if (value.indexOf('<') == -1
							&& !value.match(/(&#38;|&#160;|&#60;|&#62;)/g)) {
						var r = this.getRng();
						var n = this.getDoc().createTextNode(
								tinyMCE.entityDecode(value));
						var s = this.getSel();
						var r2 = r.cloneRange();
						s.removeAllRanges();
						r.deleteContents();
						...
						value = tinyMCE.fixGeckoBaseHREFBug(1, this.getDoc(),
								value);
		...
		case "Indent":
			...
			tinyMCE.triggerNodeChange();
			if (tinyMCE.isIE) {
				var n = tinyMCE.getParentElement(this.getFocusElement(),
						"blockquote");
				do {
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		if (tos.getEditorTemplate)
			editorTemplate = tos
					.getEditorTemplate(this.settings, this.editorId);
		deltaWidth = editorTemplate.delta_width ? editorTemplate.delta_width
				: 0;
		deltaHeight = editorTemplate.delta_height ? editorTemplate.delta_height
				: 0;
		html = '<span id="' + this.editorId
				+ '_parent" class="mceEditorContainer">' + editorTemplate.html;
		...
		if (replace_element.nodeName != "TEXTAREA"
			...
			hc = '<input type="hidden" id="' + form_element_name + '" name="'
					+ form_element_name + '" />';
			this.oldTargetDisplay = tinyMCE.getStyle(this.oldTargetElement,
					'display', 'inline');
		...
		if (dynamicIFrame) {
			html = tinyMCE.getParam('doctype')
					...
					+ tinyMCE.settings.base_href
					+ '" /><title>blank_page</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body class="mceContentBody"></body></html>';
			try {
				if (!this.isHidden())
					this.getDoc().designMode = "on";
				doc.open();
		...
		this.formElement = tinyMCE.isGecko ? parentElm.previousSibling
	...
	getHTML : function(r) {
		...
		if (r)
			return b.innerHTML;
		h = tinyMCE._cleanupHTML(this, d, this.settings, b, false, true, false,
				true);
		if (tinyMCE.getParam("convert_fonts_to_spans"))
	...
	triggerSave : function(skip_cleanup, skip_callback) {
		...
		htm = skip_cleanup ? this.getBody().innerHTML : tinyMCE._cleanupHTML(
				this, this.getDoc(), this.settings, this.getBody(),
				tinyMCE.visualAid, true, true);
		htm = tinyMCE._customCleanup(this, "submit_content", htm);
		if (!skip_callback && tinyMCE.settings.save_callback !== '')
			content = tinyMCE.resolveDots(tinyMCE.settings.save_callback,
				...
				TinyMCE_Engine,
				{
					cleanupHTMLCode : function(s) {
						...
						s = s
								.replace(
										new RegExp(
												'<(h[1-6]|p|div|address|pre|form|table|li|ol|ul|td|b|font|em|strong|i|strike|u|span|a|ul|ol|li|blockquote)([a-z]*)([^\\\\|>]*)\\/>',
												'gi'), '<$1$2$3></$1$2>');
						s = s.replace(new RegExp('\\s+></', 'gi'), '></');
						s = s.replace(new RegExp(
								'<(img|br|hr)([^>]*)><\\/(img|br|hr)>', 'gi'),
								'<$1$2 />');
						if (tinyMCE.isIE)
							s = s.replace(
									new RegExp('<p><hr \\/><\\/p>', 'gi'),
									"<hr>");
						if (tinyMCE.isIE)
							s = s.replace(/<!(\s*)\/>/g, '');
						return s
							...
							if (val != null && val !== '') {
								...
								if (val.indexOf('url(') != -1
									...
									if (m.length > 1)
										val = "url('"
												+ eval(tinyMCE
														.getParam('urlconverter_callback')
														+ "(m[1], null, true);")
												+ "')"
								}
					...
					convertSpansToFonts : function(doc) {
						var s, i, size, fSize, x, fFace, fColor, sizes = tinyMCE
								.getParam('font_size_style_values').replace(
										/\s+/, '').split(',');
						s = tinyMCE.selectElements(doc, 'span,font');
						for (i = 0; i < s.length; i++) {
							size = tinyMCE.trim(s[i].style.fontSize)
									.toLowerCase();
							fSize = 0;
							...
							if (fColor != null && fColor !== '') {
								tinyMCE.setAttrib(s[i], 'color', tinyMCE
										.convertRGBToHex(fColor));
								s[i].style.color = ''
							}
						}
					},
					convertFontsToSpans : function(doc) {
						var fsClasses, s, i, fSize, fFace, fColor, sizes = tinyMCE
								.getParam('font_size_style_values').replace(
										/\s+/, '').split(',');
					...
					_fixListElements : function(d) {
						var nl, x, a = [ 'ol', 'ul' ], i, n, p, r = new RegExp(
								'^(OL|UL)$'), np;
					...
					_fixTables : function(d) {
						...
						for (i = 0; i < nl.length; i++) {
							n = nl[i];
							if ((p = tinyMCE.getParentElement(n,
									'p,h1,h2,h3,h4,h5,h6')) != null) {
					...
					_cleanupHTML : function(inst, doc, config, elm, visual,
						...
						tinyMCE._customCleanup(inst,
								on_save ? "get_from_editor_dom"
										: "insert_to_editor_dom", doc.body);
						if (d)
							t2 = new Date().getTime();
						c.settings.on_save = on_save;
						c.idCount = 0;
						c.serializationId = new Date().getTime().toString(32);
						c.serializedNodes = [];
						c.sourceIndex = -1;
						...
						h = tinyMCE._customCleanup(inst,
								on_save ? "get_from_editor"
										: "insert_to_editor", h);
						if (on_save) {
							h = h.replace(new RegExp(' ?(mceItem[a-zA-Z0-9]*|'
						...
						if (on_save && c.settings.indent)
							h = c.formatHTML(h);
						if (on_submit
								&& (s.encoding == "xml" || s.encoding == "html"))
							h = c.xmlEncode(h);
						if (d)
							t5 = new Date().getTime();
						if (c.settings.debug)
							tinyMCE.debug("Cleanup in ms: Pre=" + (t2 - t1)
									+ ", Serialize: " + (t3 - t2) + ", Post: "
									+ (t4 - t3) + ", Format: " + (t5 - t4)
									+ ", Sum: " + (t5 - t1) + ".");
						return h
					}
...
TinyMCE_Cleanup.prototype = {
	init : function(s) {
		...
		this.nlAfterRe = this._arrayToRe(s.newline_after_elements.split(','),
				'gi', '<(', ')([^>]*)>');
		this.nlBeforeAfterRe = this._arrayToRe(s.newline_before_after_elements
				.split(','), 'gi', '<(\\/?)(', ')([^>]*)>');
		this.serializedNodes = [];
		if (s.invalid_elements !== '')
			this.iveRe = this._arrayToRe(s.invalid_elements.toUpperCase()
					.split(','), 'g', '^(', ')$');
		else
			this.iveRe = null;
		st = '';
			...
			for (y = 0; y < tn.length; y++) {
				...
				for (i = 0; i < cl.length; i++) {
					if (cl[i] == '%istrict')
						...
						r += (cl[i].charAt(0) != '#' ? cl[i].toUpperCase()
								: cl[i]);
					r += (i != cl.length - 1 ? '|' : '')
				}
				r += ')$';
					...
					for (i = 0; i < a.length; i++) {
						...
						if (av && av.length > 0) {
							if (av[0].charAt(0) == ':') {
								...
								r.forceAttribs[t.toLowerCase()] = av[0]
										.substring(1)
								...
								if (!r.validAttribValues)
									r.validAttribValues = tinyMCE
											.clearArray([]);
								r.validAttribValues[t.toLowerCase()] = this
										._arrayToRe(this.split('?', av[0]
												.substring(1)), 'i')
							}
						}
						r.vAttribsRe += '' + t.toLowerCase()
								+ (i != a.length - 1 ? '|' : '');
						a[i] = t.toLowerCase()
					}
					if (r.reqAttribsRe)
						r.reqAttribsRe = new RegExp(r.reqAttribsRe + ')=\"',
								'g');
					r.vAttribsRe += ')$';
					r.vAttribsRe = this._wildcardToRe(r.vAttribsRe);
					r.vAttribsReIsWild = new RegExp('\\*|\\?|\\+', 'g')
							.test(r.vAttribsRe);
	...
	serializeNodeAsHTML : function(n, inn) {
		...
		case 1:
			...
			if (this.vElementsRe.test(nn)
					&& (!this.iveRe || !this.iveRe.test(nn)) && !inn) {
				va = true;
				r = this.rules[nn];
				if (!r) {
					at = this.rules;
	...
	_urlConverter : function(c, n, v) {
		if (!c.settings.on_save)
			return tinyMCE.convertRelativeToAbsoluteURL(
					tinyMCE.settings.base_href, v);
		...
		.add(
				TinyMCE_Engine,
				{
					...
					getElementByAttributeValue : function(n, e, a, v) {
						return (n = this
								.getElementsByAttributeValue(n, e, a, v)).length == 0 ? null
								: n[0]
					},
					...
					setInnerHTML : function(e, h) {
						...
						if (tinyMCE.isRealIE) {
							...
							for (i = nl.length - 1; i >= 0; i--) {
								n = nl[i];
								if (n.nodeName == 'P' && !n.hasChildNodes()
										&& !n.mce_keep)
					...
					getNodeTree : function(n, na, t, nn) {
						return this.selectNodes(n, function(n) {
							return (!t || n.nodeType == t)
									&& (!nn || n.nodeName == nn)
						}, na ? na : [])
					},
					getParentElement : function(n, na, f, r) {
						var re = na ? new RegExp('^('
								+ na.toUpperCase().replace(/,/g, '|') + ')$')
								: 0, v;
					...
					getAttrib : function(elm, name, dv) {
						...
						if (name == "class" && !v)
							v = elm.className;
						if (tinyMCE.isGecko) {
							if (name == "src" && elm.src != null
									&& elm.src !== '')
								v = elm.src;
							if (name == "href" && elm.href != null
									&& elm.href !== '')
								v = elm.href
					...
					setStyleAttrib : function(e, n, v) {
						...
						if (tinyMCE.isIE && v == null || v == '') {
							v = tinyMCE.serializeStyle(tinyMCE
									.parseStyle(e.style.cssText));
							e.style.cssText = v;
					...
					switchClass : function(ei, c) {
						...
						if (tinyMCE.switchClassCache[ei])
							...
							e = tinyMCE.switchClassCache[ei] = document
									.getElementById(ei);
						if (e) {
					...
					addCSSClass : function(e, c, b) {
						...
						return e.className = b ? c
								+ (o !== '' ? (' ' + o) : '')
								: (o !== '' ? (o + ' ') : '') + c
					},
					removeCSSClass : function(e, c) {
						c = e.className.replace(new RegExp("(^|\\s+)" + c
								+ "(\\s+|$)"), ' ');
						return e.className = c != ' ' ? c : ''
					},
					hasCSSClass : function(n, c) {
						return new RegExp('\\b' + c + '\\b', 'g')
								.test(n.className)
					},
					renameElement : function(e, n, d) {
						var ne, i, ar;
						d = typeof (d) == "undefined" ? tinyMCE.selectedInstance
								.getDoc()
								: d;
						if (e) {
							...
							for (i = ar.length - 1; i > -1; i--) {
								if (ar[i].specified && ar[i].nodeValue)
									ne.setAttribute(ar[i].nodeName
											.toLowerCase(), ar[i].nodeValue)
							}
							ar = e.childNodes;
							for (i = 0; i < ar.length; i++)
					...
					getViewPort : function(w) {
						...
						return {
							left : w.pageXOffset
									|| (m ? de.scrollLeft : b.scrollLeft),
							top : w.pageYOffset
									|| (m ? de.scrollTop : b.scrollTop),
							width : w.innerWidth
									|| (m ? de.clientWidth : b.clientWidth),
							height : w.innerHeight
									|| (m ? de.clientHeight : b.clientHeight)
						}
					},
...
tinyMCE
		.add(
				TinyMCE_Engine,
				{
					parseURL : function(url_str) {
						...
						if (url_str) {
							...
							if (url_str.charAt(pos) == ':') {
								...
								urlParts.port = url_str.substring(lastPos + 1,
										pos)
							}
							lastPos = pos;
							pos = url_str.indexOf('?', lastPos);
							...
							if (url_str.charAt(pos) == '?') {
								...
								urlParts.query = url_str.substring(lastPos + 1,
										pos)
							}
							lastPos = pos;
							if (url_str.charAt(pos) == '#') {
								pos = url_str.length;
								urlParts.anchor = url_str.substring(
										lastPos + 1, pos)
							}
						}
						return urlParts
					...
					convertAbsoluteURLToRelativeURL : function(base_url,
							url_to_relative) {
						var baseURL = this.parseURL(base_url), targetURL = this
								.parseURL(url_to_relative);
						...
						if (targetURL.path == '')
							...
							forceSlash = true;
						base_url = baseURL.path.substring(0, baseURL.path
								.lastIndexOf('/'));
					...
					convertRelativeToAbsoluteURL : function(base_url,
							relative_url) {
						var baseURL = this.parseURL(base_url), baseURLParts, relURLParts, newRelURLParts, numBack, relURL = this
								.parseURL(relative_url), i;
						var len, absPath, start, end, newBaseURLParts;
						if (relative_url == ''
								|| relative_url.indexOf('://') != -1
								|| /^(mailto:|javascript:|#|\/)/
										.test(relative_url))
							return relative_url;
					...
					convertAllRelativeURLs : function(body) {
						...
						for (i = 0; i < elms.length; i++) {
							...
							if (mhref !== '')
								href = mhref;
							if (href && href !== '') {
								href = tinyMCE.convertRelativeToAbsoluteURL(
										tinyMCE.settings.base_href, href);
		...
		.add(
				TinyMCE_Engine,
				{
					_setEventsEnabled : function(node, state) {
						...
						var events = [ 'onfocus', 'onblur', 'onclick',
								'ondblclick', 'onmousedown', 'onmouseup',
								'onmouseover', 'onmousemove', 'onmouseout',
								'onkeypress', 'onkeydown', 'onkeydown',
								'onkeyup' ];
						...
						for (y = 0; y < evs.length; y++) {
							...
							for (i = 0; i < elms.length; i++) {
								...
								for (x = 0; x < events.length; x++) {
									if ((event = tinyMCE.getAttrib(elms[i],
											events[x])) !== '') {
										event = tinyMCE.cleanupEventStr(""
												+ event);
										if (!state)
					...
					setEventHandlers : function(inst, s) {
						var doc = inst.getDoc(), ie, ot, i, f = s ? tinyMCE.addEvent
								: tinyMCE.removeEvent;
						ie = [ 'keypress', 'keyup', 'keydown', 'click',
								'mouseup', 'mousedown', 'controlselect',
								'dblclick' ];
					...
					onMouseMove : function() {
						...
						if (!tinyMCE.hasMouseMoved) {
							inst = tinyMCE.selectedInstance;
							if (inst.isFocused) {
								inst.undoBookmark = inst.selection
										.getBookmark();
...
TinyMCE_Selection.prototype = {
	getSelectedHTML : function() {
		...
		else if (typeof (r.item) != 'undefined'
				|| typeof (r.htmlText) != 'undefined')
			e.innerHTML = r.item ? r.item(0).outerHTML : r.htmlText;
		else
			e.innerHTML = r.toString();
		h = tinyMCE._cleanupHTML(inst, inst.contentDocument, inst.settings, e,
				e, false, true, false);
		return h
	},
	...
	getBookmark : function(simple) {
		var inst = this.instance, rng = this.getRng(), doc = inst.getDoc(), b = inst
				.getBody();
	...
	scrollToNode : function(node) {
		var inst = this.instance, w = inst.getWin(), vp = inst.getViewPort(), pos = tinyMCE
				.getAbsPosition(node), cvp, p, cwin;
		if (pos.absLeft < vp.left || pos.absLeft > vp.left + vp.width
				|| pos.absTop < vp.top
				|| pos.absTop > vp.top + (vp.height - 25))
	...
	getAbsPosition : function(n) {
		var pos = tinyMCE.getAbsPosition(n), ipos = tinyMCE
				.getAbsPosition(this.instance.iframeElement);
		return {
...
TinyMCE_UndoRedo.prototype = {
	add : function(l) {
		...
		if (this.undoLevels[this.undoIndex]
				&& newHTML != this.undoLevels[this.undoIndex].content) {
			...
			if (customUndoLevels != -1
					&& this.undoLevels.length > customUndoLevels) {
	...
	undo : function() {
		...
		if (this.undoIndex > 0) {
			...
			if (inst.settings.custom_undo_redo_restore_selection)
				inst.selection
						.moveToBookmark(this.undoLevels[this.undoIndex].bookmark)
		}
	},
	redo : function() {
		var inst = this.instance;
...
var TinyMCE_ForceParagraphs = {
	_insertPara : function(inst, e) {
		var doc = inst.getDoc(), sel = inst.getSel(), body = inst.getBody(), win = inst.contentWindow, rng = sel
				.getRangeAt(0);
		var rootElm = doc.documentElement, blockName = "P", startNode, endNode, startBlock, endBlock;
		var rngBefore, rngAfter, direct, startNode, startOffset, endNode, endOffset, b = tinyMCE.isOpera ? inst.selection
				.getBookmark()
				: null;
		...
		direct = rngBefore.compareBoundaryPoints(rngBefore.START_TO_END,
				rngAfter) < 0;
		...
		startNode = startNode.nodeName == "BODY" ? startNode.firstChild
				: startNode;
		endNode = endNode.nodeName == "BODY" ? endNode.firstChild : endNode;
		startBlock = inst.getParentBlockElement(startNode);
		endBlock = inst.getParentBlockElement(endNode);
		if (startBlock
				&& (startBlock.nodeName == 'CAPTION' || /absolute|relative|static/gi
						.test(startBlock.style.position)))
			startBlock = null;
		if (endBlock
				&& (endBlock.nodeName == 'CAPTION' || /absolute|relative|static/gi
						.test(endBlock.style.position)))
			endBlock = null;
		if (startBlock != null) {
			blockName = startBlock.nodeName;
			if (/(TD|TABLE|TH|CAPTION)/.test(blockName)
					|| (blockName == "DIV" && /left|right/gi
							.test(startBlock.style.cssFloat)))
				blockName = "P"
		}
		...
		paraBefore = (startBlock != null && startBlock.nodeName == blockName) ? startBlock
				.cloneNode(false)
				: doc.createElement(blockName);
		paraAfter = (endBlock != null && endBlock.nodeName == blockName) ? endBlock
				.cloneNode(false)
				: doc.createElement(blockName);
		...
		node = startChop;
		do {
			if (node == body || node.nodeType == 9
					|| tinyMCE.isBlockElement(node))
				break;
			startChop = node
		} while ((node = node.previousSibling ? node.previousSibling
				: node.parentNode));
		node = endChop;
		do {
			if (node == body || node.nodeType == 9
		...
		if (startBlock == null) {
			...
			if (startChop != rootElm && endChop != rootElm) {
				...
				if (contents.firstChild
						&& (contents.firstChild.nodeName == blockName || contents.firstChild.nodeName == "BODY"))
				...
				body.innerHTML = "<" + blockName + ">&nbsp;</" + blockName
						+ "><" + blockName + ">&nbsp;</" + blockName + ">";
		...
		if (!startChop.previousSibling
				&& startChop.parentNode.nodeName.toUpperCase() == blockName) {
		...
		if (!endChop.nextSibling
				&& endChop.parentNode.nodeName.toUpperCase() == blockName)
...
TinyMCE_Layer.prototype = {
	...
	resizeBy : function(w, h) {
		var e = this.getElement();
		this
				.resizeTo(parseInt(e.style.width) + w, parseInt(e.style.height)
						+ h)
	},
...
TinyMCE_Menu.prototype = tinyMCE
		.extend(
				TinyMCE_Layer.prototype,
				{
					init : function(s) {
					...
					update : function() {
						...
						for (i = 0; i < m.length; i++) {
							...
							default:
								h += '<tr><td><a href="'
										+ tinyMCE.xmlEncode(m[i].js)
										+ '" onmousedown="'
										+ tinyMCE.xmlEncode(m[i].js)
										+ ';return tinyMCE.cancelEvent(event);" onclick="return tinyMCE.cancelEvent(event);" onmouseup="return tinyMCE.cancelEvent(event);"><span'
										+ c + '>' + t + '</span></a>'
							}
							h += '</td></tr>'
						}
