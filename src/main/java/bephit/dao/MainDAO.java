...
public class MainDAO {
	...
	public String getmaxparticipantid() {
		...
		try {
			String cmd;
			resultSet = statement
					.executeQuery("select  max(participants_id) as participant from participants_table;");
	...
	public int updateparticipantmessage(ParticipantsDetails participant,
		...
		try {
			...
			if (participant.getMessage() != null)
				cmd = "UPDATE participants_table SET message ='"
						+ participant.getMessage()
						+ "' WHERE participants_id='" + participants_id + "'";
			else
				cmd = "UPDATE participants_table SET message ='0' WHERE participants_id='"
						+ participants_id + "'";
			logger.info(cmd);
	...
	public int checkuser(String admin_username) {
		...
		try {
			...
			String cmd_userlist = "Select count(*) as counting from login where username='"
					+ admin_username + "'";
	...
	public int checksecondaryemail(String admin_email, int from,
		...
		try {
			...
			if (from == 0)
				cmd_emaillist = "Select count(*) as counting from login where email_id='"
						+ admin_email
						+ "' or secondary_email='"
						+ admin_email
						+ "'";
			...
			/*
			 * else { ResultSet rs=statement.executeQuery(
			 * "Select count(*) as counting from participants_table where email_id='"
			 * +admin_email+"' and participant_id!='"+participantid+"'" );
			 * rs.next(); int
			 * count1=Integer.parseInt(resultSet.getString("counting"));
		...
		try {
			...
			if (from == 0)
				...
				cmd_emaillist = "Select count(*) as counting from login where email_id='"
						...
						+ "' and username!='"
						+ participantid
						+ "' or secondary_email='" + admin_email + "'";
			logger.info("maillist" + cmd_emaillist);
			resultSet = statement.executeQuery(cmd_emaillist);
			resultSet.next();
			int count = Integer.parseInt(resultSet.getString("counting"));
	...
	public int checkmobile(String admin_mobile) {
		...
		try {
			...
			String cmd_mobilelist = "Select count(*) as counting from participants_table where mobile_num='"
					+ admin_mobile + "'";
	...
	public int setregisterParticipants(ParticipantsDetails participant,
			String admin_id, String[] groups, int from) {
		Connection con = null;
		Statement statement = null;
		ResultSet resultSet = null;
		int flag = 0;
		...
		try {
			...
			String cmd_login = "insert into login(username,password,email_id,secondary_email,role,status) values('"
					+ participant.getUsername()
					+ "','"
					+ pw
					+ "','"
					+ participant.getEmail_id() + "','null',0,1)";
			statement.execute(cmd);
			statement.execute(cmd_login);
			/* statement.execute(cmd); */

			String cmd_getid = "SELECT login_id from login where username='"
					+ participant.getUsername() + "'";
			logger.info(cmd_login);
			/*
			 * String Desc="added adminuser"+admin_id; String cmd_activity=
			 * "insert into admin_log_activity_table(admin_id,ip_address,admin_date_time,admin_desc) values('"
			 * +
			 * admin_id+"','127.0.0.1','"+dateFormat.format(date)+"','"+Desc+"')"
			 * ; System.out.println(cmd_activity);
			 */
			resultSet = statement.executeQuery(cmd_getid);
			resultSet.next();
			int lastinsertedid = Integer.parseInt(resultSet
					.getString("login_id"));

			String cmd_role = "insert into user_roles(user_id,authority) values('"
					+ lastinsertedid + "','ROLE_USER')";
			statement.execute(cmd_role);
			
			
			//Insert into weekly logs
			...
			// Weekly logs
			if (!participant.getWeekly_survey_start_date().equals("")) {
				today_date = formatter.parseDateTime(participant
						.getWeekly_survey_start_date());
				System.out.println("Formatted Date:"
						+ today_date.toString("yyyy-MM-dd"));
				insertWeeklylog(today_date.toString("yyyy-MM-dd"));
			}			
			//End Weekly logs
			...
			try {
				...
				// Email Test
				emailSender.password_sendEmail(participant.getEmail_id(),
						"adherencemedicationproject",
						"Adherence Project Registration",
						participant.getFname(), participant.getUsername(), pw);
			...
			/*
			 * if(merror==1) {
			 * statement.executeUpdate("delete from login where username='"
			 * +participant.getUsername()+"'"); statement.executeUpdate(
			 * "delete from participants_table where username='"
			 * +participant.getUsername()+"'"); String
			 * error="delete from login where username='"
			 * +participant.getUsername()+"'"; statement.executeUpdate(
			 * "delete from participant_table where username='"
			 * +participant.getUsername()+"'"); System.out.println(error);
			 * 
			 * 
			 * }
			 */
			 ...
			 * ().getPrincipal(); UserDetails userDetails = null; if (principal
			 * instanceof UserDetails) { userDetails = (UserDetails) principal;
			 * } String userName = userDetails.getUsername();
			 */
			if (resultSet.next()) {
				participants = resultSet.getString("participant");
			...
			for (String group : groups) {
				resultSet = statement
						.executeQuery("select group_id,group_name from participant_group_table where group_name='"
								+ group + "'");
				int i = 0;
			...
			for (int i = 0; i < count; i++) {
				String cmd_mess = "insert into participant_group(group_id,group_name,participant_id) values('"
						+ strlist.get(i)
						+ "','"
						+ strlist1.get(i)
						+ "','"
			 ...
			 */
			/*
			 * InetAddress IP=InetAddress.getByName("127.0.0.1"); try { IP =
			 * InetAddress.getLocalHost();
			 * //System.out.println("IP of my system is := "
			 * +IP.getHostAddress()); } catch (UnknownHostException e) { // TODO
			 * Auto-generated catch block e.printStackTrace(); }
			 */

			/*
			 * String cmd_activity =
			 * "insert into admin_log_activity_table(admin_id,ip_address,admin_date_time,admin_desc,done_by) values('"
			 * + admin_id + "','"+IP.getHostAddress()+"','" +
			 * dateFormat.format(date) + "','" + Desc + "','"+providername+"')";
			 * System.out.println(cmd_activity);
			 * statement.execute(cmd_activity);
			 */
			flag = 1;
		...
		try {
			...
			try {
				...
				emailSender.password_sendEmail(participant.getEmail_id(),
						"adherencemedicationproject",
						"Adherence Project Registration",
						participant.getFname(), participant.getUsername(),
						participant.getPassword());
	...
	public int updateregisterParticipants(ParticipantsDetails participant,
			String participants_id, String providername, String[] groups,
			int from) {
		Connection con = null;
		Statement statement = null;
		...
		try {
			...
			List<String> strlist1 = new ArrayList<String>();
			statement
					.executeUpdate("delete from participant_group where participant_id='"
							+ participants_id + "'");
			for (String group : groups) {
				resultSet = statement
						.executeQuery("select group_id,group_name from participant_group_table where group_name='"
			...
			String cmd = "UPDATE participants_table SET id='"
					...
					+ participant.getEducation() + "',medical_details = '"
					+ participant.getMedical_details() + "',time1='"
					+ participant.getTime1() + "',time1_am_pm='"
					+ participant.getTime1_am_pm() + "',time2='"
					+ participant.getTime2() + "',time2_am_pm='"
					...
					+ participants_id + "';";
			String cmd_login = "UPDATE login SET email_id='"
					+ participant.getEmail_id() + "' where username= '"
					+ participant.getUsername() + "'";
			logger.info(cmd);
			logger.info(cmd_activity);

			
			//Check weekly log status
			System.out.println("Check weekly_logs inserted or not");
			String cmd_weekly_log="select * from weekly_logs where participant_id='"+participants_id+"'";
			boolean isInserted=false;
			resultSet=statement.executeQuery(cmd_weekly_log);
			if(resultSet.next())
			{
			...
			if(!isInserted)
			{
				System.out.println("Not Inserted Already!!");
				DateTimeFormatter formatter = DateTimeFormat.forPattern("M/d/Y");
				//
				// --------------------------Time
				// Zone-----------------------------//
...
if(mobilenumber.length()!=0)
{
			...
			String cmd = "UPDATE participants_table SET id='"
					+ participant.getId() + "', fname ='"
					+ participant.getFname() + "',username ='"
					+ participant.getUsername() + "',password='"
					+ participant.getPassword() + "',mobile_num ='"
					+ participant.getMobile_num() + "',gender ='"
					+ participant.getGender() + "'  ,city ='"
					+ participant.getCity() + "' ,education = '"
					+ participant.getEducation() + "',medical_details = '"
					...
					+ participant.getTime2_am_pm() + "',time3='"
					+ participant.getTime3() + "',time3_am_pm='"
					+ participant.getTime3_am_pm() + "',startdate='"
					+ participant.getStartdate()
					+ "',weekly_survey_start_date='"
					+ participant.getWeekly_survey_start_date()
					+ "',provider_email1='"
					...
					+ participants_id + "';";
			String cmd_login = "UPDATE login SET email_id='"
					+ participant.getEmail_id() + "',password='"
					+ participant.getPassword() + "' where username= '"
					+ participant.getUsername() + "'";
			logger.info(cmd);
			logger.info(cmd_activity);
			logger.info(cmd_login);
	...
	public String getproviders(String participantid) {
		...
		try {
			...
			cmd = "select * from participants_table where participants_id='"
					+ participantid + "'";
	...
	public List<ParticipantsDetails> getParticipants() {
		...
		try {
			...
			if (userName.equals("superadmin"))
				...
				cmd = "select * from participants_table where Provider_name='"
						+ userName + "' order by fname asc";
			...
			while (resultSet.next()) {
				participants.add(new ParticipantsDetails(resultSet
						...
						.getString("id"), resultSet.getString("fname"),
	...
	public int sendForgotpassword(String email) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				logger.info("--Before Sending--"); // Logger Test
				// Email Test
				emailSender.password_sendEmail(email, "learnguild.com",
						"Forgot Password", resultSet.getString("username"),
	...
	public List<ParticipantsDetails> getlimitedParticipants(int page) {
		...
		try {
			...
			int limit = 20;
			if (userName.equals("superadmin"))
				cmd = "select * from participants_table  order by fname asc limit "
						+ offset + "," + limit + "";
			else
				cmd = "select * from participants_table where provider_name='"
						+ userName + "'  order by fname asc limit " + offset
						+ "," + limit + "";
	...
	public int getnoofParticipants() {
		...
		try {
			...
			if (userName.equals("superadmin"))
				...
				cmd = "select count(*) as noofrecords from participants_table where provider_name='"
						+ userName + "'";
			logger.info(cmd);
			resultSet = statement.executeQuery(cmd);
			if (resultSet.next())
	...
	public int deleteParticipant(String participant_id, String admin) {
		...
		try {
			...
			String messagelog = "delete from participant_message_log where Participant_id='"
					+ participant_id + "'";

			statement.execute(messagelog);

			// 2.Delete From group mapping table
			String participant_group = "delete from participant_group where participant_id='"
					+ participant_id + "'";

			statement.execute(participant_group);

			String selectedparticipantusername = "select username from participants_table where participants_id='"
					+ participant_id + "'";

			String participantusername = "", user_id = "";
			logger.info("username" + selectedparticipantusername);
			ResultSet resultSet1 = statement
					.executeQuery(selectedparticipantusername);
			while (resultSet1.next()) {
				participantusername = resultSet1.getString("username");
			}
			String selectedloginid = "select login_id from login where username='"
					+ participantusername + "'";
			...
			String user_roles = "delete from user_roles where USER_ID='"
					+ user_id + "'";
			statement.execute(user_roles);
			logger.info(user_roles);

			// 4.Delete from login table[important]
			String login = "delete from login where username='"
					+ participantusername + "'";
			statement.execute(login);
			logger.info(login);

			logger.info("select log_id from weekly_logs where participant_id='"
					+ participant_id + "'");

			ResultSet resultset4 = statement
					.executeQuery("select log_id from weekly_logs where participant_id='"
							+ participant_id + "'");
			...
			for (String week_id : weeklog_id) {
				...
				statement.execute("delete from weekly_answers where log_id='"
						+ week_id + "'");

			}

			// 6.Delete from weekly logs
			statement.execute("delete from weekly_logs where participant_id='"
					+ participant_id + "'");

			// 7.Delete from participant table
			String participant_table = "delete from participants_table where participants_id='"
					+ participant_id + "'";
			statement.execute(participant_table);
			logger.info(participant_table);

			// 8.Delete from weekly audio
			String cmd_delete_weekly_audio = "Delete from weekly_audio where participant_id='"
					+ participant_id + "'";
		...
		try {
			...
			if (!mobile.equals("") && !groupname.equals("") && !city.equals("")) {
				if (userName.equals("superadmin"))
					cmd = "select * from participants_table where mobile_num='"
							+ mobile + "' or group_name like '%" + groupname
							+ "%' or city='" + city + "'";
				else
					cmd = "select * from participants_table where mobile_num='"
							+ mobile + "' or group_name like '%" + groupname
							+ "%' or city='" + city
							+ "' having Provider_name='" + userName + "'";
			}
			if (mobile.equals("") && !groupname.equals("") && city.equals("")) {
				if (userName.equals("superadmin"))
					cmd = "select * from participants_table where group_name like '%"
							+ groupname + "%'";
				else
					cmd = "select * from participants_table where group_name like '%"
							+ groupname
							+ "%' having Provider_name='"
							+ userName + "'";
			...
			if (!mobile.equals("") && groupname.equals("") && city.equals("")) {
				if (userName.equals("superadmin"))
					...
					cmd = "select * from participants_table where mobile_num='"
							+ mobile + "' having Provider_name='" + userName
							+ "'";

			}
			if (mobile.equals("") && groupname.equals("") && !city.equals("")) {
				if (userName.equals("superadmin"))
					cmd = "select * from participants_table where city='"
							+ city + "'";
				else
					cmd = "select * from participants_table where city='"
			...
			if (mobile.equals("") && !groupname.equals("") && !city.equals("")) {
				if (userName.equals("superadmin"))
					cmd = "select * from participants_table where city='"
							+ city + "' or group_name like '%" + groupname
							+ "%'";
				else
					cmd = "select * from participants_table where city='"
							+ city + "' or group_name like '%" + groupname
			...
			if (!mobile.equals("") && !groupname.equals("") && city.equals("")) {
				if (userName.equals("superadmin"))
					cmd = "select * from participants_table where group_name like '%"
							+ groupname + "%' or mobile_num='" + mobile + "'";
				else
					cmd = "select * from participants_table where group_name like '%"
							+ groupname
							+ "%' or mobile_num='"
	...
	public int getrole() {
		...
		// ArrayList<ParticipantsDetails>();
		try {
			String cmd_role = "select role from login where username='"
					+ userName + "'";
	...
	public String getparticipantid() {
		...
		// ArrayList<ParticipantsDetails>();
		try {
			String cmd_role = "select participants_id from participants_table where username='"
					+ userName + "'";
			resultSet = statement.executeQuery(cmd_role);
			resultSet.next();
	...
	public int checkuser(String admin_username, int from, String participantid) {
		...
		try {
			...
			String cmd_userlist = "";
			if (from == 0)
				cmd_userlist = "Select count(*) as counting from login where username='"
						+ admin_username + "'";
			else if (from == 1)
				cmd_userlist = "Select count(*) as counting from login where username='"
						+ admin_username + "'";
	...
	public int checkmobile(String admin_mobile, int from, String participantid) {
		...
		try {
			...
			if (from == 0)
				cmd_mobilelist = "Select count(*) as counting from participants_table where mobile_num='"
						+ admin_mobile + "'";
			else if (from == 1)
				cmd_mobilelist = "Select count(*) as counting from participants_table where mobile_num='"
						+ admin_mobile
	...
	public int insertWeeklylog(String date) {
		Connection con = null;
		Statement statement = null;
		ResultSet resultSet = null;
		int flag = 0;
		...
		try {
			...
			for (int i = 1; i <= 12; i++) {
				...
				cmd = "insert into weekly_logs(participant_id,week,date_time,continous,count,status) values('"
						+ pid
						+ "','"
						+ i
						+ "',DATE_ADD('"
						+ date
						+ "', INTERVAL '" + days + "' DAY),'0','0','0')";
				statement.execute(cmd);
				days = days + 7;
